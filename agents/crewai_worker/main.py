...
from mappers import extract_agent_metadata, map_kickoff_to_crew_inputs
...
def main() -> int:
    ...
    def _maybe_vote(ctx) -> None:
        ...
        try:
            ...
            logger.info(
                "compliance response sent proposalId=%s vote=%s recommendation=%s",
                pending["proposal_id"], vote, pending["recommendation"],
            )
        except MacpAckError as err:
            if _is_session_closed(err):
                logger.info(
                    "compliance vote skipped — session already closed proposalId=%s err=%s",
    ...
    def handle_proposal(message, ctx):
        emit_signal(
            ctx.actions,
            "session.started",
            {"role": agent_meta["role"] or "compliance-agent", "framework": "crewai", "agentRef": "compliance-agent"},
        )
